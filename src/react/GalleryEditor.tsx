...
export function GalleryEditorContent({ slug, gallery: initialGallery }: Props) {
  ...
  const handleSaveImage = (imageUrl: string) => {
    ...
    const tile = gallery.layout.find(
      (t) => t.id === layout[selectedTileIndex].i
    );
    if (tile) {
  ...
  const handleDeleteTile = async () => {
    ...
    const tileToDelete = gallery.layout.find(
      (tile) => tile.id === layout[selectedTileIndex].i
    );
    ...
    if (isHomeGallery && tileToDelete?.link) {
      ...
      if (gallerySlug) {
        try {
          ...
          setToast({
            message: "Failed to delete associated gallery",
            type: "error",
          });
          return;
        }
    ...
    const updatedLayout = layout.filter(
      (item) => item.i !== layout[selectedTileIndex].i
    );
    setLayout(updatedLayout);

    // Update gallery layout
    const updatedGalleryLayout = gallery.layout.filter(
      (tile) => tile.id !== layout[selectedTileIndex].i
    );
    gallery.layout = updatedGalleryLayout;
    ...
    try {
      ...
      for (const [index, file] of files.entries()) {
        ...
        if (!file.type.startsWith("image/")) {
          setToast({
            message: `Skipped ${file.name}: Not an image file`,
            type: "error",
          });
          continue;
        }
        ...
        if (file.size > 50 * 1024 * 1024) {
          setToast({
            message: `Skipped ${file.name}: File too large`,
            type: "error",
          });
          continue;
        }
  ...
  const handleDeleteAll = () => {
    ...
    if (
      window.confirm(
        "Are you sure you want to delete all tiles? This action cannot be undone."
      )
    ) {
      // Clear layout
      setLayout([]);
  ...
  return (
    <div className="w-full px-[50px]">
      ...
      <div className="fixed bottom-0 z-200 bg-zinc-800/50 backdrop-blur-sm left-0 right-0 bg-white border-t border-zinc-500 p-4 flex justify-between shadow-lg shadow-black/20">
              ...
              {!isHomeGallery && (
                <>
                  <div className="flex flex-col items-center">
                    ...
                    <span className="text-[10px] mt-1 text-white">Upload Images</span>
                  </div>
                  <div className="flex flex-col items-center">
                    <button
        ...
        <div className="flex gap-2">
          ...
          <div className="flex flex-col items-center">
            <button
              onClick={handleSave}
              disabled={!hasChanges || loadingStates['save']}
              ...
              title="Save Changes"
            >
              {loadingStates['save'] && <LoadingSpinner />}
              Save Changes
            </button>
      ...
      <TextEditor
        ...
        initialText={
          selectedTileIndex !== null
            ? gallery?.layout.find((t) => t.id === layout[selectedTileIndex]?.i)
                ?.description || ""
            : ""
        }
      />
